	...
	constructor(game, options){
		...
		loader.load( `${game.assetsPath}fbx/people/${model}.fbx`, function ( object ) {
			...
			if (player.local){
				...
				const player = game.initialisingPlayers.splice(game.initialisingPlayers.indexOf(this), 1);
				game.remotePlayers.push(player[0]);
			}
...
class PlayerLocal extends Player{
	constructor(game, model){
		...
		$('form').submit(function(){
			socket.emit('chat message',$('#text_chat').val());
			$('#text_chat').val('');
			return false
		});
